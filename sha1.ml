...
(* Left-rotate *)
let rol (value: int) (bits: int) : int = ((value lsl bits) lor (value lsr (32 - bits))) land 0xffffffff
...
let number_of_32_bit_be (b: bytes) : int=
  let open Bytes in
  (Char.code (get b 0)) lsl 24 + (Char.code (get b 1)) lsl 16 +
  (Char.code (get b 2) lsl 8) + (Char.code (get b 3))
...
let digest (bytes: bytes) : bytes =
  ...
  let preprocess _a =
    ...
    let length_to_add = (if (512 - length_mod_512) >= 65
                         then (512 - length_mod_512)
                         else (512 - length_mod_512 + 512)) in
    let to_append = Bytes.make (length_to_add / 8) '\x00' in
    Bytes.set to_append 0 '\x80';
    Bytes.blit (number_to_32_bit_be ml) 0 to_append (Bytes.length to_append - 4) 4;
    Bytes.concat (Bytes.of_string "") [_a; to_append] in
  ...
  for i = 0 to (preprocessed_length / 64) - 1 do
    ...
    let w = Array.init 80 (fun k -> if k < 16
                            then number_of_32_bit_be (Bytes.sub chunk (k * 4) 4)
                            else 0) in
    for j = 16 to 79 do
      w.(j) <- rol (w.(j - 3) lxor w.(j-8) lxor w.(j-14) lxor w.(j - 16)) 1
    done;
    let a = ref !h0 in
    let b = ref !h1 in
    let c = ref !h2 in
    ...
    for j = 0 to 79 do
      if (0 <= j && j < 20) then (
        ...
        k := 0x5A827999;
      ) else if (20 <= j && j < 40) then (
        f := !b lxor !c lxor !d;
        k := 0x6ED9EBA1;
      ) else if (40 <= j && j < 60) then (
  ...
  Bytes.concat (Bytes.of_string "") (List.map number_to_32_bit_be [!h0; !h1; !h2; !h3; !h4])
